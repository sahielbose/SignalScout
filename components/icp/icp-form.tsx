...
export function IcpForm({ action, initial, submitLabel = 'Save ICP', onCancel }: IcpFormProps) {
  ...
  return (
    <form action={action} className="space-y-4">
        ...
        <div className="flex flex-wrap gap-1.5">
          {SIGNAL_TYPES.map((t) => {
            ...
            return (
              <label
                key={t}
                className="cursor-pointer select-none rounded-full border px-3 py-1 text-xs font-medium text-muted-foreground transition-colors has-[:checked]:border-primary/40 has-[:checked]:bg-primary/15 has-[:checked]:text-primary"
              >
      ...
      <div className="flex flex-wrap items-center gap-6 rounded-md border bg-muted/30 p-3">
