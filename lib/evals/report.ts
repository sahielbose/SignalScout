...
export async function runClassificationEval(): Promise<EvalReport> {
  // Disable llm_runs persistence for the duration of the eval, then ALWAYS
  // restore it in finally - otherwise an early return/throw leaves logging off
  // process-wide and cost recording silently stops for every later request.
  try {
    setLlmLogging(false);
